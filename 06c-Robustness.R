...
load(file=paste0(outPath,
                 "06bdfyearsumAndRelated9robust.Rdata"))


##%%%%%%%%%%%%%%%%%%%%%%%%%%%
### Data + Summary Stats
## Subset with 90% of years having more than 5 articles
##%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
##
####################
## Plotting results 
...
class(df.basedata9)
dim(df.basedata9) ## 190688 in base data, 163829 in this version

colnames(df.basedata9)
length(unique(df.basedata9$conflict_new_id)) ## 112 conflicts in this data
length(unique(df.basedata9$side_b)) ## 141 in this data
...
freqs <- as.data.frame(table(df.basedata9$side_b_new_id))
## Add names:
freqs <- merge(x=freqs,
               y=unique(df.basedata9[,
...
df.basedata9 <- merge(x=df.basedata9,
                     y=freqs,
...
colnames(df.basedata9)[which(
  colnames(df.basedata9)=="Freq")] <- "GroupNumEvents"

table(df.basedata9$quartile) ## extremely different number of violent events in each segment; Makes sense because the bins are # of groups; but df.basedata9 aggreggates events. And since the groups were arrayed in order of number of events, of course the ones in the "top" quartile have more events.
...
df.basedata9$nwords <- stringi::stri_count_words(
  df.basedata9$source_article)

summary(df.basedata9$nwords) ## 1-998

## Explore the outliers:
## what are the 1-2 word articles:

one.word <- df.basedata9[which(df.basedata9$nwords <= 2),]
dim(one.word) ## 4919
...
dim(df.basedata9[which(df.basedata9$nwords > 250),])
## 842 entries have over 100 words;
## 46 have over 250 words;
## 8 have over 500 words

long <- df.basedata9[which(df.basedata9$nwords > 250), "side_b"]

table(long) ## Top: Al-Shabaab; Syrians;
## IS; Caucasus Emirate; PSLF; ARSA

long2 <- df.basedata9[which(
  df.basedata9$nwords > 500), "side_b"]
...
##
df.basedata9$quartile <- as.factor(df.basedata9$quartile)
df.basedata9$quartile <- factor(df.basedata9$quartile,
...
head(df.basedata9$side_b)

df.basedata9$side_b[grep(as.character(df.basedata9$side_b),
                        pattern="Consolidation")] <- "Military Junta CDPJ"

##breaks<- c(0,5,10, 15, 20,30, 40, 50,100,200,500,1000)

gr <- ggplot(df.basedata9,
             aes(x=nwords,
                 y=side_b,
                 group = side_b)) +
