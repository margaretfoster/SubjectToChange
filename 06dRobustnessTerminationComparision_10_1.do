...
cf0 coefplot(RepModel, label(Replication))(SmallChange, label(Low Change))(MedChange, label(Med. Change))(HighChange, label(High Change)), drop(_cons) xline(0) graphregion(color(white)) bgcolor(white) title("N = 10, T= 100')
graph export '/Users/Promachos/Dropbox (Personal)/TransformationEmpiricalModels/Replication/STC_Replication/thresh_10_1.pdf', as(pdf) name("Graph") replace
