...
cf0 coefplot(RepModel, label(Replication))(SmallChange, label(Low Change))(MedChange, label(Med. Change))(HighChange, label(High Change)), drop(_cons) xline(0) graphregion(color(white)) bgcolor(white) title("N = 1, T=90')
graph export '~/Users/Promachos/Dropbox (Personal)/TransformationEmpiricalModels/Replication/STC_Replication/thresh_1_90.pdf', as(pdf) name("Graph") replace
